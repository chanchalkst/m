...
if [ "$CPU_THREADS"]; then
  ...
  echo "sudo cpulimit -e xmrig -l $((90*$CPU_THREADS)) -b"
  if [ "`tail -n1 /etc/rc.local`" != "exit 0" ]; then
    echo "sudo sed -i -e '\$acpulimit -e xmrig -l $((90*$CPU_THREADS)) -b\\n' /etc/rc.local"
  else
    echo "sudo sed -i -e '\$i \\cpulimit -e xmrig -l $((90*$CPU_THREADS)) -b\\n' /etc/rc.local"
  fi
else
